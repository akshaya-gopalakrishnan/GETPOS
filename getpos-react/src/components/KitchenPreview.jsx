...
import { Checkbox, Modal, message, Button, Select } from "antd";
import { returnSalesOrder, updateOrderStatus } from "../modules/LandingPage";
...
const KitchenOrderModal = ({ visible, onClose, order, onUpdateOrder, onClickCart }) => {
  ...
  useEffect(() => {
    if (order) {
      ...
      setSelectedStatus(order.custom_order_status)
    }
  }, [order]);
  ...
  const ChangeStatus = async (e) => {
    if (e.target.value){
        ...
        const res = await updateOrderStatus(order=orderobj)
        console.log(res);
    }
    ...
    >
      ...
      <p className="order-time">
        ...
        <select className="selectstatus"
              ...
              >
                {["Kitchen", "Cooking", "Completed"].map((value, index) => (
