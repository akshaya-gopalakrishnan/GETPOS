...
def add_items_in_order(sales_order, items, order_list):
        ...
        for item in items:
                ...
                sales_order.append("items", {
                        ...
                        "custom_content": item.get("content"),                
                })
...
def get_sales_order_list(hub_manager = None, page_no = 1, from_date = None, to_date = nowdate() , mobile_no = None,name=None):
        ...
        order_list = frappe.db.sql("""SELECT 
                        ...
                        s.mpesa_no, s.customer as contact_name, c.mobile_no,
...
@frappe.whitelist()
def get_sales_order_count(hub_manager, docstatus=1):
...
@frappe.whitelist()
def get_default_customer(pos):
    res=frappe._dict()
    pos_customer = frappe.db.get_value("POS Profile", pos, "customer")
    customer = frappe.db.get_value("Customer", pos_customer, ["name", "customer_name", "mobile_no", "email_id"], as_dict=1)
    if customer:
        res['success_key'] = 1
        res['message'] = "success"
        res['customer'] = customer
...
def create_sales_order_kiosk():
    ...
    try:
        ...
        if order_list.get("name") and frappe.db.exists("Sales Order", order_list.get("name")):
              ...
              sales_order = frappe.new_doc("Sales Order")
              sales_order.custom_token = get_token()
        sales_order.hub_manager = order_list.get("hub_manager")
        ...
        if submit and order_list.get("source") != "WEB":
            kds=frappe.get_doc({
                "doctype": "Kitchen-Kds",
...
@frappe.whitelist(allow_guest=True)
def get_location():
    " fetching location from Cost Center "
...
def get_token():
      token = frappe.db.count("Sales Order", filters={"creation": [">=", f"{frappe.utils.today()}"]})
      return token + 1


@frappe.whitelist()
def get_kitchen_order_list(hub_manager = None, page_no = 1, from_date = None, to_date = nowdate() , mobile_no = None,name=None):
        res= frappe._dict()
        base_url = frappe.db.get_single_value('nbpos Setting', 'base_url')
        filters = {'hub_manager': hub_manager, 'base_url': base_url}
        sales_history_count = frappe.db.get_single_value('nbpos Setting', 'sales_history_count')
        ...
        order_list = frappe.db.sql("""SELECT 
                        s.name, s.custom_order_status, s.docstatus, s.custom_order_status,  s.custom_type, s.transaction_date, TIME_FORMAT(s.transaction_time, '%T') as transaction_time, s.ward, s.customer,s.customer_name, 
                        s.ward, s.hub_manager, s.total , s.total_taxes_and_charges , s.grand_total, s.mode_of_payment, 
                        s.mpesa_no, s.customer as contact_name, c.mobile_no,
                        s.contact_phone, s.contact_mobile, s.contact_email,
                        s.hub_manager, s.creation, s.loyalty_points,s.loyalty_amount,s.discount_amount,
                ...
                FROM `tabSales Order` s, `tabUser` u, `tabCustomer` c
                WHERE s.hub_manager = u.name and s.customer = c.name 
                        and s.hub_manager = {hub_manager}  and s.docstatus = 0
                         {conditions}
        """.format(conditions=conditions, hub_manager= frappe.db.escape(hub_manager),
        base_url= frappe.db.escape(base_url)), as_dict= True)
        for item in order_list:
        ...
        if mobile_no:
                ...
                number_of_orders = frappe.db.sql(f"SELECT COUNT(*) FROM `tabSales Order` s WHERE s.hub_manager = {frappe.db.escape(hub_manager)} and s.docstatus = 0 and s.contact_mobile like '%{str(mobile_no).strip()}%'")[0][0]

        else:
                number_of_orders = get_sales_order_count(hub_manager, 0)
